...
describe('budget manager create budget', () => {
  beforeEach(async () => {
    // creates a fresh pinia and make it active so it's automatically picked
    // up by any useStore() call without having to pass it to it:
    // `useStore(pinia)`
  ...
  it('creates budget', async () => {
    await budgetHelper.createBudget('test - budget')
    expect(budgetmanager.masterCategories.length).toBe(5)
    expect(budgetmanager.categories.length).toBe(30)
    expect(budgetmanager.accounts.length).toBe(0)
    expect(budgetmanager.budgetData.length).toBe(32)
  })

  it('create category', async () => {
    const num_of_categories = budgetmanager.categories.length
    await budgetHelper.createCategory('5c6ac657-0a34-4592-a174-a0e6b79bc596', 'test - category', '601d8401-bce3-4d1f-95ce-1af2bdd9f285')

    expect(budgetmanager.categories.length).toBe(num_of_categories + 1)
    expect(budgetmanager.categories[0].name).toBe('test - category')
  })
    
  it('initialize categories', async () => {
    const num_of_categories = budgetmanager.categories.length
    const num_of_master_categories = budgetmanager.masterCategories.length

    const b_id = uuidv4()
    await budgetHelper.initializeBudgetCategories(b_id)

    expect(budgetmanager.masterCategories.length).toBe(num_of_master_categories + 5)
    expect(budgetmanager.categories.length).toBe(num_of_categories+20)
