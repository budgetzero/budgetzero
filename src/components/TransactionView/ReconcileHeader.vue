<template>
  ...
  >
    ...
    <BaseDialogModalComponent v-model="isModalVisibleForReconcileConfirmation">
      ...
      <template #body>
        <span class="subtitle-1">There is a difference between your cleared balance and your bank balance. 
          If you continue this will create an adjustment transaction of <v-chip>{{ -parseInt(differenceAmount) | currency }}</v-chip></span>
      </template>
      <template #actions>
        <v-spacer />
        <v-btn
          color="grey"
          text
          @click.stop="isModalVisibleForReconcileConfirmation = false"
        >
          Go Back
        </v-btn>
        <v-btn
          id="btn-completeReconcile"
          color="accent"
          @click="addReconciliationTransaction()"
        >
          Confirm
        </v-btn>
      </template>
    ...
    >
      ...
      >
        <v-card-title class="title font-weight-bold primary--text">
          {{ reconcileAmount | currency }}<br></span>
        </v-card-title>
        <v-card-subtitle>
    ...
    >
      ...
      >
        <v-card-title class="title font-weight-bold primary--text">
          {{ differenceAmount | currency }}<br></span>
        </v-card-title>
        <v-card-subtitle>
...
export default {
  ...
  computed: {
    ...
    selected_account_balance() {
      return this.$store.getters.account_balances[
